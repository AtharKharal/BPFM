...
```{r}
skm_original %>% filter(complete_rate<0.999) %>% 
  ...
  arrange(CompletenessRate) %>% huxtable::as_huxtable(add_colnames=T)
```
...
It is a almost balanced dataset in terms of classes:
```{r}
table(dt$label)
```
