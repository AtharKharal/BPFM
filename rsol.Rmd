...
## Vital Statistics for Numerical Variables  
  
```{r}
ExpNumStat(dt,by="G",gp="label",Qnt=seq(0,1,0.1),MesofShape=2,Outlier=TRUE,round=2)
```

### Information Value of Variables  

```{r}
ExpCatStat(dt,Target="label",result = "IV",clim=10,nlim=5,bins=10,Pclass="Yes",plot=FALSE,top=20,Round=2)
```

```{r}
ExpCatStat(dt,Target="label",result = "Stat",clim=10,nlim=5,bins=10,Pclass="Yes",plot=FALSE,top=20,Round=2)
```
...
```{r}
ExpCatStat(dt,Target = "label")
```

```{r}
ExpCatViz(dt, target = "label", col=c("springgreen1", "red1"))
```
